"""Create a v2 annotation JSON from LCC model write_raster prediction outputs.
...
4. Emits a v2 annotation entry (a 128x128 window centered on the pixel, with one
   ...
   ``rslp.olmoearth_lcc.annotation_app.create_windows``.

Usage::

    python -m rslp.olmoearth_lcc.annotation_scripts.phase02_output_sampling.create_v2_annotations \
...
from rslp.olmoearth_lcc.lcc_model.postprocess import (
    BINARY_CHANGE_BAND,
    DST_BAND_OFFSET,
    LC_CLASS_NAMES,
...
from rslp.olmoearth_lcc.lcc_model.timestamp_encoding import days_to_date
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Create a v2 annotation JSON from LCC model write_raster outputs."
    )
    parser.add_argument(
        "--input_dir",
        required=True,
