...
from .per_pixel_model import PerPixelModelWrapper


class MultiLayerSentinel2Normalize(Transform):
    """Apply OlmoEarth Sentinel-2 normalization to arbitrarily-named Sentinel-2 inputs.

    ``OlmoEarthNormalize`` expects the input to be keyed ``sentinel2_l2a``; this wraps
    it so each of the per-year ``sentinel2_yN`` layers can be normalized in turn.
    """
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        """Create a new MultiLayerSentinel2Normalize.

        Args:
            modality_names: the input keys to normalize.
            band_names: the Sentinel-2 band order of those inputs.
            kwargs: forwarded to ``OlmoEarthNormalize``.
        """
        super().__init__()
    ...
    def forward(
        ...
        """Apply OlmoEarthNormalize Sentinel-2 normalization on each configured key."""
...
class PassthroughTask(Task):
    """Task with no targets; all inputs are passthrough."""
    ...
    def __iter__(self) -> Iterator[Any]:
        ...
        for idx in indices:
            ...
            try:
                ...
                print(
                    "[land_cover.find_change] skipping window "
...
def _build_model_dataset(
    ...
    normalizer = MultiLayerSentinel2Normalize(
