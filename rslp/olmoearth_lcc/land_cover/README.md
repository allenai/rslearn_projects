...
Training the per-pixel land cover model
---------------------------------------

`per_pixel_model.py` provides `PerPixelModelWrapper`, which runs a spatial rslearn
model independently on each 1x1 pixel so the resulting land cover model has no
spatial context. It is trained on the WorldCover dataset (see `rslp/worldcover/`)
with:

    rslearn model fit --config data/olmoearth_lcc/land_cover/config_per_pixel.yaml

Applying it to find candidate changes
-------------------------------------

`find_change.py` applies the per-pixel land-cover model to a ten-year Sentinel-2
dataset (see the main README for how that dataset is created) and, for each window,
picks one qualifying land-cover decline (a class whose probability stays above
`--src_threshold` in the three years before a pivot year and falls below
`--dst_threshold` in the three years after). It writes one V2-compatible
...
Example local usage:
    ...
    python -m rslp.olmoearth_lcc.land_cover.find_change \
...
checkpoint, and output directory can live on `/weka`):
    ...
    python -m rslp.main common beaker_launcher \
        ...
        --command '["python", "-m", "rslp.olmoearth_lcc.land_cover.find_change", "--ds_path", "/weka/dfive-default/rslearn-eai/datasets/change_finder/ten_year_dataset_20260408/", "--checkpoint_path", "/weka/dfive-default/rslearn-eai/projects/2026_04_05_worldcover_change/olmoearth_base_s2_center10_per_pixel_ps1_01/best.ckpt", "--output_dir", "/weka/dfive-default/path/to/output_jsons/", "--workers", "32"]'
