"""Sample v2 annotation candidates from the Codex-generated tiles CSVs."""
...
class Tile:
    """One WGS84 tile from a codex_tiles CSV."""
...
def _load_tiles(tiles_csv: Path) -> list[Tile]:
    """Load tile rows from a codex_tiles CSV."""
...
def _tile_metadata(tile: Tile, point_idx: int, source: str) -> dict[str, Any]:
...
def _make_entry(
    ...
    return {
        ...
        "negative_points": [],
        "metadata": _tile_metadata(tile, point_idx, source),
    }
...
def sample_entries(
    ...
    source = f"olmoearth_lcc/codex_tiles/{tiles_csv.name}"
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Sample random points from a codex_tiles CSV and write a shuffled "
            "v2 annotation JSON."
        )
    )
    parser.add_argument(
        "--tiles-csv",
        type=Path,
        default=script_dir / "codex_tiles_1.csv",
        help="Input tile CSV. Default: codex_tiles_1.csv next to this script.",
    )
    parser.add_argument(
        "--output",
        type=Path,
