"""Create ten-year dataset windows restricted to a set of African countries.

This mirrors :mod:`rslp.olmoearth_lcc.ten_year_dataset.create_windows` but only keeps windows
...
_CACHE_PATH = (
    Path.home() / ".cache" / "olmoearth_lcc" / "ne_50m_admin_0_countries.geojson"
)
...
def _load_country_geometry() -> (
    ...
    if not _CACHE_PATH.exists():
        ...
        resp = requests.get(
            NE_COUNTRIES_URL, headers={"User-Agent": "olmoearth-lcc"}, timeout=60
        )
...
def create_windows_africa(
    ...
    Args:
        ds_path: output dataset path, e.g.
            /weka/dfive-default/rslearn-eai/datasets/change_finder/ten_year_dataset_20260408/
