"""Write Sentinel-1 vessel detection tasks to a Beaker queue."""
...
def write_entries(
    ...
    """Write Sentinel-1 vessel detection tasks to Beaker queue.

    Note that the prediction pipeline requires every scene in a batch to share the same
    orbit direction, so ``json_fname`` should contain scene IDs from a single orbit
    direction.

    Args:
        queue_name: the Beaker queue to write to.
        json_fname: the JSON filename containing a list of string Sentinel-1 scene IDs
            to process.
        ...
        batch_size: how many Sentinel-1 scenes to process in parallel on one GPU.
        score_threshold: the detector score threshold to apply for this run, overriding
    ...
    write_jobs(queue_name, "sentinel1_vessels", "predict", args_list)
