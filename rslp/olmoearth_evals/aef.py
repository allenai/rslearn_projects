"""Evaluation adapter for OlmoEarth."""

from typing import Any

import torch
from rslearn.models.component import IntermediateComponent
from rslearn.models.conv import Conv
from rslearn.models.faster_rcnn import FasterRCNN
from rslearn.models.feature_center_crop import FeatureCenterCrop
from rslearn.models.module_wrapper import EncoderModuleWrapper
from rslearn.models.multitask import MultiTaskModel
from rslearn.models.pooling_decoder import PoolingDecoder
from rslearn.train.model_context import ModelContext
from rslearn.train.tasks.classification import ClassificationHead
from rslearn.train.tasks.regression import RegressionHead
from rslearn.train.tasks.segmentation import SegmentationHead
from rslearn.train.transforms import Sequential
from rslearn.train.transforms.normalize import Normalize

from rslp.nandi.train import SegmentationPoolingDecoder


class Identity(IntermediateComponent):
    """Identity IntermediateComponent that passes through intermediates unchanged."""

    def forward(self, intermediates: Any, context: ModelContext) -> Any:
        """Pass through intermediates unchanged.

        Args:
            intermediates: the output from the previous component.
            context: the model context (ignored).

        Returns:
            unchanged intermediates
        """
        return intermediates


def get_model(
    input_size: int,
    input_modalities: list[str],
    task_type: str,
    task_name: str,
    task_channels: int = 1,
    task_timesteps: int = 1,
) -> torch.nn.Module:
    """Get appropriate AEF model, which mirrors the OlmoEarth model."""
    if task_type == "segment":
        decoders = dict(
            eval_task=[
                Conv(
                    in_channels=64,
                    out_channels=256,
                    kernel_size=3,
                ),
                Conv(
                    in_channels=256,
                    out_channels=256,
                    kernel_size=3,
                ),
                Conv(
                    in_channels=256,
                    out_channels=256,
                    kernel_size=3,
                ),
                Conv(
                    in_channels=256,
                    out_channels=task_channels,
                    kernel_size=3,
                    activation=torch.nn.Identity(),
                ),
                SegmentationHead(),
            ]
        )
    elif task_type == "segment_small":
        decoders = dict(
            eval_task=[
                SegmentationPoolingDecoder(
                    in_channels=64,
                    out_channels=task_channels,
                ),
                SegmentationHead(),
            ]
        )
    elif task_type == "detect":
        decoders = dict(
            eval_task=[
                FasterRCNN(
                    downsample_factors=[1],
                    num_channels=64,
                    num_classes=task_channels,
                    anchor_sizes=[[32]],
                )
            ]
        )
    elif task_type == "classify":
        decoders = dict(
            eval_task=[
                Conv(
                    in_channels=64,
                    out_channels=256,
                    kernel_size=3,
                ),
                Conv(
                    in_channels=256,
                    out_channels=256,
                    kernel_size=3,
                ),
                Conv(
                    in_channels=256,
                    out_channels=256,
                    kernel_size=3,
                ),
                FeatureCenterCrop(
                    sizes=[[1, 1]],
                ),
                PoolingDecoder(
                    in_channels=256,
                    out_channels=task_channels,
                    num_fc_layers=1,
                ),
                ClassificationHead(),
            ]
        )
    elif task_type == "regress":
        decoders = dict(
            eval_task=[
                Conv(
                    in_channels=64,
                    out_channels=256,
                    kernel_size=3,
                ),
                Conv(
                    in_channels=256,
                    out_channels=256,
                    kernel_size=3,
                ),
                Conv(
                    in_channels=256,
                    out_channels=256,
                    kernel_size=3,
                ),
                FeatureCenterCrop(
                    sizes=[[1, 1]],
                ),
                PoolingDecoder(
                    in_channels=256,
                    out_channels=task_channels,
                    num_fc_layers=1,
                ),
                RegressionHead(),
            ]
        )
    else:
        raise NotImplementedError

    return MultiTaskModel(
        encoder=[EncoderModuleWrapper(module=Identity())],
        decoders=decoders,
    )


def get_transform(
    input_size: int,
    input_modalities: list[str],
    task_type: str,
    task_name: str,
    task_channels: int = 1,
    task_timesteps: int = 1,
) -> torch.nn.Module:
    """Get appropriate OlmoEarth transform."""
    modules: list[torch.nn.Module] = []

    if (len(input_modalities) != 1) or ("gse" not in input_modalities[0]):
        # we use '"gse" not in' instead of == because
        # the layer name can be a slight variant of gse, e.g. 'gsegood'
        raise ValueError("GSE model only works with gse input")

    modules.append(Normalize(mean=0, std=16384))

    return Sequential(*modules)
