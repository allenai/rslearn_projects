...
def fwi_agg(
    ...
    """Aggregate FWI xarray data to a NetCDF or GeoTIFF export."""
    ...
    for date in temp_grid_dates:
        end_date = date + timedelta(days=temp_offset - 1)  # bounds are inclusive
        ds_dc_fwi = tot_ds_dc_fwi.sel(valid_time=slice(date, end_date))
        ds_dc_fwi_mean = ds_dc_fwi.mean(dim="valid_time")  # Skip Missing Values
        ds_dc_fwi_min = ds_dc_fwi.min(dim="valid_time")  # Skip Missing Values
        ds_dc_fwi_max = ds_dc_fwi.max(dim="valid_time")  # Skip Missing Values
        ds_dc_fwi = xr.merge(
            [
            ...
            compat="override",  # Explicitly set compat to silence FutureWarning
        )
        ds_dc_fwi = ds_dc_fwi.assign_coords(valid_time=date)
        tot_list_ds.append(ds_dc_fwi)

    print("Concatnating aggregated values....")
    ...
    transform = from_origin(
        west - (res / 2), north + (res / 2), res, res
    )  # Not top corner without the extra shift
    tot_ds_dc_fwi.rio.write_transform(transform, inplace=True)

    # Sorting the data by longitude/latitude is key for the rasterio saving process
    ...
    if store:
        ...
        # Storing the data as .nc files xarray Dataset
        tot_ds_dc_fwi.to_netcdf(file_path.parent / f"fwi_dc_agg_{year}.nc")

        # Storing the data as .tif files for each date
        for date in temp_grid_dates:
