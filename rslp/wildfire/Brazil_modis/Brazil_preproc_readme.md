...
```shell
cd ./rslp/wildfire/Brazil_modis && \
python -m data_preproc_script.preprocess.create_grid \
    --grid-type adaptive \
    --output-path ./datasets/MCD64A1-brazil/FIREd_Brazil_preproc/grid/Brazil_3200m_adaptive \
    --min-x -74 \
    --max-x -34 \
    --max-y 5.3 \
    --min-y -33.8 \
...
Column renaming (previously a separate `rename_fields` step) is now handled
automatically via `fire_id_col`, `start_date_col`, and `end_date_col` in
`ba_preprocess.yaml`.
...
Config: `configs/sampling_unified.yaml`.
