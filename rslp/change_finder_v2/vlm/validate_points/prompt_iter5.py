"""Iteration 5: iter4 + stronger same-source corroboration to cut cross-source FPs.

The dominant false-positive mode in iter1/iter3 is the model treating a single
high-resolution aerial image (a different sensor, season, and resolution than
Sentinel-2) as evidence of change relative to the Sentinel-2 series. iter5 keeps iter4's
center box and cloud-aware Sentinel-2 selection, and adds explicit rules:

- a change must be corroborated WITHIN one source's own time series (Sentinel-2 before
  vs Sentinel-2 after, or aerial vs aerial) - never inferred from an aerial-vs-Sentinel-2
  difference alone;
- aerial images are primarily for reading WHAT the land cover is at high detail; the
  Sentinel-2 series is the timeline that must show the before and after states;
- if the only evidence is a difference between an aerial image and Sentinel-2 images,
  answer negative.
...
def _build_prompt() -> str:
    lines = [
        ...
        "",
        "MOST IMPORTANT RULE - avoid false alarms from comparing different sources: a "
        "real change must be corroborated WITHIN A SINGLE SOURCE'S OWN time series - "
        "Sentinel-2 before vs Sentinel-2 after, OR aerial vs aerial. NEVER conclude "
        "change from an aerial-vs-Sentinel-2 difference alone: an aerial image looking "
        "different from a Sentinel-2 image is expected (different sensor, season, "
        "resolution) and is NOT evidence of change. If the only difference you can find "
        "is between an aerial image and the Sentinel-2 images, answer 'negative'.",
        "",
        "Also do NOT call a change based on any of these (they are not land-cover "
        "change):",
        "  - clouds, cloud shadows, haze, smoke, or dust;",
        "  - snow or ice cover, frozen ground, or changing water level / sun glint on "
        "water;",
        "  - overall brightness/color/contrast differences between dates;",
        ...
        "  2. it is corroborated within the Sentinel-2 timeline (or within multiple "
        "aerial images): the post-change state APPEARS and then PERSISTS in multiple "
        "later, reasonably CLEAR Sentinel-2 images (not a one-image blip, and not based "
        "only on an aerial-vs-Sentinel-2 difference);",
        ...
        "box. Use the FULL range and make it graded: ~0.9-1.0 only when the change is "
        "obvious and corroborated across multiple clear Sentinel-2 images, ~0.5-0.8 "
        "when plausible but partly obscured or weakly supported, ~0.2-0.4 for only a "
        "hint, ~0.0-0.1 for clearly stable scenes. Avoid defaulting to exactly 0 or 1 "
...
def build(record, metas, load_array):
    """Build the iter5 prompt and image list for one point."""
