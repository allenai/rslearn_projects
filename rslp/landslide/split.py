...
val_percentage = 2  # 20% for validation

for window in tqdm.tqdm(windows):
    # Convert hash to integer and use modulo for consistent splitting
    hash_val = int(hashlib.sha256(window.name.encode()).hexdigest(), 16)
    
    if hash_val % 100 < val_percentage:
        split = "val"
    else:
        split = "train"
