...
glc_csv_path = os.path.join(
    data_root, "glc/Global_Landslide_Catalog_Export_20250201.csv"
)
df = pd.read_csv(
    glc_csv_path,
    encoding="utf-8",
...
shp_path = os.path.join(
    data_root,
    "nsidc/HMA_MTLI_1-20260106_221858/HMA_MTLI_v01_landslide_inventory/HMA_MTLI_v01_Footprint.shp",
)
...
events = gdf[
    ["id", "event_date", "latitude", "longitude", "location", "event_type", "geometry"]
].copy()
...
for year in years:
    ...
    ax.set_title(
        f"Global Landslide Events in {year}\nSen12Landslides Dataset",
        fontsize=14,
        fontweight="bold",
    )
...
if max_nearby > 0:
    ...
    print(
        f"  Location with most neighbors: lat={max_coord[0]:.4f}, lon={max_coord[1]:.4f}"
    )
...
valid_events["lat_grid"] = (
    valid_events["latitude"] / grid_resolution
).round() * grid_resolution
valid_events["lon_grid"] = (
    valid_events["longitude"] / grid_resolution
).round() * grid_resolution

# Count events per grid cell
grid_counts = (
    valid_events.groupby(["lat_grid", "lon_grid"]).size().reset_index(name="count")
)
...
print(
    f"  2-5 events: {((grid_counts['count'] >= 2) & (grid_counts['count'] <= 5)).sum():,} cells"
)
print(
    f"  6-10 events: {((grid_counts['count'] >= 6) & (grid_counts['count'] <= 10)).sum():,} cells"
)
