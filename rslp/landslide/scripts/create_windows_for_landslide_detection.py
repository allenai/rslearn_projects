...
python create_windows_for_landslide_detection.py \
    ...
    --max_samples 1  # for testing with subset
"""
...
def create_window_pair(
    ...
    if sample_type == "negative" or sample_type == "positive":
        ...
        # Create features for all overlapping landslides in negative window
        if len(negative_overlapping) > 0:
            # If there are landslides, label them
            negative_features = []
            for landslide in negative_overlapping:
                label_geometry = STGeometry(WGS84_PROJECTION, landslide["geometry"], None)
                feature = Feature(
                    label_geometry,
                    {
                        "label": "landslide",
            ...
            # No landslides - label as no_landslide
    ...
    if sample_type == "positive":
        ...
        # Create features for all overlapping landslides
        positive_features = []
        for landslide in positive_overlapping:
            label_geometry = STGeometry(WGS84_PROJECTION, landslide["geometry"], None)
            feature = Feature(
                label_geometry,
                {
                    "label": "landslide",
                    "landslide_id": str(landslide["id"]),  # Ensure string type
                    "event_type": str(landslide["event_type"]),
                    "event_date": str(landslide["event_date"]),
                    "is_primary": bool(landslide["id"] == sample_id),  # Mark which one centered the window
                },
            )
