    ...
    with tempfile.NamedTemporaryFile(mode="w") as f:
        swa_config = {
            "trainer": {
                "callbacks+": [
                    {
                        "class_path": "lightning.pytorch.callbacks.stochastic_weight_avg.StochasticWeightAveraging",
                        "init_args": {
                            "swa_lrs": 0.000075,
                            "swa_epoch_start": 50,
                        }
        ...
        yaml.dump(swa_config, f)
        f.flush()
        cmd.append(f"--config_paths+={f.name}")

        print()
        print("SWA_CONFIG")
        print(json.dumps(swa_config, indent=4))
        print(" ".join(cmd))
