"""Create rslearn windows for the Surface Fuels (FBFM40) dataset.

Reads /tmp/surface_fuels.csv, creates 64x64 windows at 10m in UTM with:
- label_raster: 64x64 GeoTIFF with class index at center pixel, 255 elsewhere
- label: vector layer with {"category": str(fbfm40)}
- Split: 50/25/25 train/val/test via hash of window name
- Time range: Jan 1 to Dec 31, 2024
"""
...
CSV_PATH = "/weka/dfive-default/rslearn-eai/datasets/olmoearth_evals/surface_fuels/data.csv"
DST = Path(
    "/weka/dfive-default/rslearn-eai/datasets/olmoearth_evals/surface_fuels"
)
...
CLASS_TO_IDX = {c: i for i, c in enumerate(FBFM40_CLASSES)}
...
def main() -> None:
    ...
    # Copy config.json
    config_src = Path(__file__).parent / "surface_fuels_config.json"
    import shutil
