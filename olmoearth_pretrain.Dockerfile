...
RUN --mount=type=cache,target=/root/.cache/uv uv pip install --system /opt/rslearn[extra,terratorch] /opt/olmoearth_pretrain -r /opt/rslearn_projects/requirements.txt -r /opt/rslearn_projects/requirements-extra.txt -r /opt/rslearn_projects/requirements-olmocore.txt
...
RUN --mount=type=cache,target=/root/.cache/uv uv pip install --system /opt/rslearn[extra,terratorch] /opt/olmoearth_pretrain /opt/rslearn_projects[extra]
