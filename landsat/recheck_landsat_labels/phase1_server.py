...
def get_example(idx):
    ...
    with open(os.path.join(ds_dir, "windows", "selected", example["crop_window_id"], "layers", "label", "data.geojson")) as f:
...
def get_image(example_idx):
    ...
    image_fname = os.path.join(ds_dir, "windows", "selected", example["crop_window_id"], "layers", "landsat", "R_G_B", "image.png")
    return send_file(image_fname)

@app.route('/update/<idx>', methods=['POST'])
def update(idx):
    example = examples[int(idx)]
    label_fname = os.path.join(ds_dir, "windows", "selected", example["crop_window_id"], "layers", "label", "data.geojson")
